  ...
  return (
    <AdminLayout title="Location Hierarchy Management" subtitle="Manage States, Districts, Talukas, Parganas, and Villages (State ➔ District ➔ Taluka ➔ Pargana ➔ Village)">
      <div className="space-y-6 pb-12">
        {/* Navigation Tabs */}
        <div className="bg-[#0A162D] border border-[#D4AF37]/30 rounded-2xl p-2 flex flex-wrap gap-2 shadow-xl">
          {[
            ...
            <button
              ...
              className={`flex-1 min-w-[130px] py-3 px-4 rounded-xl text-xs sm:text-sm font-extrabold transition-all cursor-pointer flex items-center justify-center gap-2 ${
                activeTab === tab.id
                  ...
                  : "text-gray-300 hover:bg-[#040C1A] hover:text-[#D4AF37]"
              }`}
            >
              <span>{tab.label}</span>
              {tab.count > 0 && (
                <span className={`px-2 py-0.5 rounded-full text-[10px] ${activeTab === tab.id ? "bg-black/20 text-black" : "bg-[#D4AF37]/20 text-[#D4AF37]"}`}>
                  {tab.count}
                </span>
              )}
            </button>
        ...
        {/* Action Header Bar */}
        <div className="bg-[#0A162D] border border-[#D4AF37]/30 rounded-2xl p-4 flex flex-col sm:flex-row gap-4 justify-between items-center shadow-2xl">
          <div className="relative flex-1 w-full">
            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-[#D4AF37]/70 text-sm">🔍</div>
            <input
              type="text"
              placeholder={`Search ${activeTab} by name or Gujarati title...`}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl pl-10 pr-4 py-2.5 text-xs sm:text-sm text-white placeholder-gray-400 focus:outline-none focus:border-[#D4AF37]"
            />
          </div>
        ...
        {/* Grid List */}
        {loading ? (
          <div className="p-12 text-center bg-[#0A162D]/60 rounded-2xl border border-[#D4AF37]/20">
            <div className="w-10 h-10 border-3 border-[#D4AF37] border-t-transparent rounded-full animate-spin mx-auto mb-3"></div>
            <p className="text-[#D4AF37] text-xs font-bold uppercase tracking-widest">Loading {activeTab} from database...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            {activeTab === "states" &&
              states
                ...
                .map((s) => (
                  <div key={s.id} className="bg-gradient-to-b from-[#0A162D] to-[#040C1A] border border-[#D4AF37]/30 hover:border-[#D4AF37] rounded-2xl p-5 shadow-xl flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h4 className="text-base font-extrabold text-[#D4AF37]">{s.name}</h4>
                      ...
                      <div className="text-xs space-y-1.5 border-t border-[#D4AF37]/20 pt-3 my-3">
                        <div className="flex justify-between text-gray-400"><span>State Code:</span> <strong className="text-white">{s.code}</strong></div>
                        <div className="flex justify-between text-gray-400"><span>Districts Count:</span> <strong className="text-[#D4AF37]">{s._count?.districts || 0}</strong></div>
                      </div>
                    </div>
                    <div className="flex gap-2 pt-2 border-t border-[#D4AF37]/20">
                      <button onClick={() => openEditModal(s)} className="flex-1 py-2 rounded-xl bg-[#040C1A] border border-[#D4AF37]/40 text-[#D4AF37] text-xs font-bold hover:bg-[#D4AF37] hover:text-black transition-all">Edit</button>
            ...
            {activeTab === "districts" &&
              districts
                ...
                .map((d) => (
                  <div key={d.id} className="bg-gradient-to-b from-[#0A162D] to-[#040C1A] border border-[#D4AF37]/30 hover:border-[#D4AF37] rounded-2xl p-5 shadow-xl flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h4 className="text-base font-extrabold text-[#D4AF37]">{d.name}</h4>
                      ...
                      <div className="text-xs space-y-1.5 border-t border-[#D4AF37]/20 pt-3 my-3">
                        ...
                        <div className="flex justify-between text-gray-400"><span>Talukas Count:</span> <strong className="text-[#D4AF37]">{d._count?.talukas || 0}</strong></div>
                      </div>
                    </div>
                    <div className="flex gap-2 pt-2 border-t border-[#D4AF37]/20">
                      <button onClick={() => openEditModal(d)} className="flex-1 py-2 rounded-xl bg-[#040C1A] border border-[#D4AF37]/40 text-[#D4AF37] text-xs font-bold hover:bg-[#D4AF37] hover:text-black transition-all">Edit</button>
            ...
            {activeTab === "talukas" &&
              talukas
                ...
                .map((t) => (
                  <div key={t.id} className="bg-gradient-to-b from-[#0A162D] to-[#040C1A] border border-[#D4AF37]/30 hover:border-[#D4AF37] rounded-2xl p-5 shadow-xl flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h4 className="text-base font-extrabold text-[#D4AF37]">{t.name}</h4>
                      ...
                      <div className="text-xs space-y-1.5 border-t border-[#D4AF37]/20 pt-3 my-3">
                        ...
                        <div className="flex justify-between text-gray-400"><span>Villages Count:</span> <strong className="text-[#D4AF37]">{t._count?.villages || 0}</strong></div>
                      </div>
                    </div>
                    <div className="flex gap-2 pt-2 border-t border-[#D4AF37]/20">
                      <button onClick={() => openEditModal(t)} className="flex-1 py-2 rounded-xl bg-[#040C1A] border border-[#D4AF37]/40 text-[#D4AF37] text-xs font-bold hover:bg-[#D4AF37] hover:text-black transition-all">Edit</button>
            ...
            {activeTab === "parganas" &&
              parganas.map((p) => (
                <div key={p.id} className="bg-gradient-to-b from-[#0A162D] to-[#040C1A] border border-[#D4AF37]/30 hover:border-[#D4AF37] rounded-2xl p-5 shadow-xl flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      <div>
                    ...
                    <div className="text-xs space-y-1.5 border-t border-[#D4AF37]/20 pt-3 my-3">
                      <div className="flex justify-between text-gray-400"><span>Calculated Villages:</span> <strong className="text-[#D4AF37]">{p.computedVillageCount || p.villageCount || "N/A"}</strong></div>
            ...
            {activeTab === "villages" &&
              villages.map((v) => (
                <div key={v.id} className="bg-gradient-to-b from-[#0A162D] to-[#040C1A] border border-[#D4AF37]/30 hover:border-[#D4AF37] rounded-2xl p-5 shadow-xl flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="text-base font-extrabold text-[#D4AF37]">{v.name}</h4>
                    ...
                    <div className="text-xs space-y-1.5 border-t border-[#D4AF37]/20 pt-3 my-3">
                      ...
                      {v.pincode && <div className="flex justify-between text-gray-400"><span>Pincode:</span> <strong className="text-[#D4AF37]">{v.pincode}</strong></div>}
                    </div>
                  </div>
                  <div className="flex gap-2 pt-2 border-t border-[#D4AF37]/20">
                    <button onClick={() => openEditModal(v)} className="flex-1 py-2 rounded-xl bg-[#040C1A] border border-[#D4AF37]/40 text-[#D4AF37] text-xs font-bold hover:bg-[#D4AF37] hover:text-black transition-all">Edit</button>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0A162D] border border-[#D4AF37]/50 rounded-2xl p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              {activeTab === "districts" && (
                <div>
                  ...
                  <select value={formStateId} onChange={(e) => setFormStateId(e.target.value)} className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl px-3.5 py-2.5 text-white">
              ...
              {activeTab === "talukas" && (
                <div>
                  ...
                  <select value={formDistrictId} onChange={(e) => setFormDistrictId(e.target.value)} className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl px-3.5 py-2.5 text-white">
              ...
              {activeTab === "villages" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={formParganaId} onChange={(e) => setFormParganaId(e.target.value)} className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl px-3.5 py-2.5 text-white">
                  ...
                  <div>
                    ...
                    <select value={formTalukaId} onChange={(e) => setFormTalukaId(e.target.value)} className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl px-3.5 py-2.5 text-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" required placeholder="Name in English" value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl px-3.5 py-2.5 text-white" />
                </div>
                <div>
                  <label className="block text-gray-300 font-semibold mb-1">Gujarati Name (ગુજરાતી)</label>
                  <input type="text" placeholder="નામ ગુજરાતીમાં" value={formGujaratiName} onChange={(e) => setFormGujaratiName(e.target.value)} className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl px-3.5 py-2.5 text-white" />
                </div>
              </div>
              ...
              {activeTab === "villages" && (
                <div>
                  ...
                  <input type="text" placeholder="e.g. 383430" value={formPincode} onChange={(e) => setFormPincode(e.target.value)} className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl px-3.5 py-2.5 text-white" />
                </div>
              )}
