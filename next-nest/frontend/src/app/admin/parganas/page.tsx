  ...
  return (
    <AdminLayout title="Pargana Management" subtitle="Manage Vankar Samaj regional divisions, village counts, Gujarati titles & contacts">
      <div className="space-y-6 pb-12">
        {/* Top Control Bar */}
        <div className="bg-[#0A162D] border border-[#D4AF37]/30 rounded-2xl p-4 sm:p-5 shadow-2xl flex flex-col sm:flex-row gap-4 justify-between items-center backdrop-blur-md">
          <div className="relative flex-1 w-full">
            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-[#D4AF37]/70 text-sm">
              🔍
            </div>
            <input
              type="text"
              ...
              className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl pl-10 pr-4 py-2.5 text-xs sm:text-sm text-white placeholder-gray-400 focus:outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37] transition-all"
            />
          </div>
        ...
        {loading ? (
          <div className="p-12 text-center bg-[#0A162D]/60 rounded-2xl border border-[#D4AF37]/20">
            ...
            <p className="text-[#D4AF37] text-xs font-bold uppercase tracking-widest">Loading pargana divisions from API...</p>
          </div>
        ) : filteredParganas.length === 0 ? (
          <div className="p-12 text-center bg-[#0A162D]/60 rounded-2xl border border-[#D4AF37]/20 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredParganas.map((p) => (
              <div
                key={p.id}
                className="bg-gradient-to-b from-[#0A162D] to-[#040C1A] border border-[#D4AF37]/30 hover:border-[#D4AF37] rounded-2xl p-5 shadow-xl hover:shadow-[#D4AF37]/10 transition-all duration-300 flex flex-col justify-between group"
              >
                <div>
                  {/* Card Header */}
                  <div className="flex justify-between items-start gap-3 mb-3 pb-3 border-b border-[#D4AF37]/20">
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-xl bg-[#040C1A] border border-[#D4AF37]/40 flex items-center justify-center text-lg shadow-inner shrink-0 group-hover:scale-105 transition-all">
                        🏛️
                      </div>
                      <div>
                        <h4 className="text-base font-extrabold text-[#D4AF37] leading-tight">{p.name}</h4>
                        {p.gujaratiName && (
                  ...
                  <div className="space-y-2 text-xs py-2">
                    {(p.computedVillageCount || p.villageCount) && (
                      <div className="flex justify-between items-center py-1 px-2.5 rounded-lg bg-[#040C1A]/60 border border-[#D4AF37]/10">
                        <span className="text-gray-400">Village Count (ગામ):</span>
                        <span className="text-[#D4AF37] font-bold">{p.computedVillageCount || p.villageCount}</span>
                      </div>
                    )}
                ...
                {/* Card Action Buttons */}
                <div className="flex gap-2 pt-4 mt-2 border-t border-[#D4AF37]/20">
                  <button
                    onClick={() => openEdit(p)}
                    className="flex-1 py-2 px-3 rounded-xl bg-[#040C1A] border border-[#D4AF37]/40 text-[#D4AF37] text-xs font-bold hover:bg-[#D4AF37] hover:text-black transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-sm"
                  >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0A162D] border border-[#D4AF37]/50 rounded-2xl p-6 w-full max-w-lg space-y-5 max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center pb-3 border-b border-[#D4AF37]/20">
              <h3 className="text-lg font-extrabold text-[#D4AF37] flex items-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#040C1A] border border-[#D4AF37]/40 rounded-xl px-3.5 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-[#D4AF37]"
                  />
                </div>
              ...
              <div className="flex items-center gap-3 pt-2 p-3 bg-[#040C1A]/80 rounded-xl border border-[#D4AF37]/20">
                <input
                  type="checkbox"
                  id="isActiveToggle"
